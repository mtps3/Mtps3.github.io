...
Before we provide our input, we can send the server values which are different to TO_SIGN, lets call them TO_SIGN', and the server respons with the signature (sig = sign(SECRET + TO_SIGN')).
...
This is a different input for the signature algorithm, but leads to the same signature :)
One remark here is that we came up with this solution when the CTF was already over.

One is that we change exactly 1 byte in TO_SIGN (to the value of the current index), let the server sign it, and store the signature in a map.
The key of this map is the signature, and the value is the occurrence of this signature.
After 128 iterations (length of TO_SIGN) we take the signature which occurred most often, and this was in almost every case the same signature :) 
This is the solution we used at the CTF.
## Solution in python
The second solution, what we used during the CTF, is in the outcommented part of the main function.
{% highlight python %}
#!/usr/bin/python
import hashlib
import itertools
